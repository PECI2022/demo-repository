...
class Operations:
    ...
    def upload(self):
        ...
        data = {"name": description['name'], "video_class": video_class, "length": video_length, "_id": str(
            _id), "update": datetime.now(), "MediaPipeHand": 0}
        # mongo_cli.insert_data(data,_id,"info")
    ...
    def average_characteristics(self):
        ...
        for video in project_video:
            blob = mongo_cli.generate_from_db(ObjectId(video['_id']))
            average_characteristics['contrast'] += data_characteristics.getContrast(blob)
            average_characteristics['brightness'] += data_characteristics.getBrightness(blob)
            average_characteristics['sharpness'] += data_characteristics.getSharpness(blob)
            average_characteristics['saturation'] += data_characteristics.getSaturation(blob)
            average_characteristics['hue'] += data_characteristics.getHue(blob)

        average_characteristics['contrast'] /= len(project_video)
        average_characteristics['brightness'] /= len(project_video)
        average_characteristics['sharpness'] /= len(project_video)
        average_characteristics['saturation'] /= len(project_video)
        average_characteristics['hue'] /= len(project_video)

        print("AVG",average_characteristics)
...
@app.route('/average_characteristics', methods=['POST'])
